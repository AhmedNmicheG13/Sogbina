...
<head>
    ...
    <title>{{ $settings->seo_title ?? 'CovoitGo - Trouvez un Covoiturage' }}</title>
    <link rel="icon" href="{{ asset('images/' . $settings->favicon) }}" type="image/x-icon">
...
.show {
    display: block;
        ...
        padding: 100px 0;
        text-align: left;
        color: {{ $settings->site_text_color ?? '#FFF' }};
        background-color: #222;
        display: flex;
        justify-content: space-between;
        align-items: center;
        background-color: {{ $settings->header_color_large ?? '#222' }};
        background-image: url('{{ asset('images/' . $settings->header_image) }}');
        ...
        .search-button:hover {
            background-color: {{ $settings->buton_color ?? '#222' }};
        }
        .ui-datepicker {
            font-size: 16px;
            background-color: #fff;
    ...
    <div class="hero custom-hero">
        <div class="container">
            <div class="text-container">
                <h1>{{ $settings->header_title ?? 'TROUVEZ UN <span class="bold">COVOITURAGE</span>' }}</h1>
                <h2>{{ $settings->header_subtitle ?? 'Covoiturez sur tous vos types de trajets sans aucune commission.' }}</h2>
            </div>
        </div>
    </div>
